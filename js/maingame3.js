...
const playerJumpSprite = new Image(); playerJumpSprite.src = 'img/capyidle3.png';

const playerIdleSprite = new Image(); playerIdleSprite.src = 'img/capyidle_jump3.png';
...
platforms = [
    ...
    ...Array(300).fill(null).map((_, index) => {
        return new Platform({
            x: 900 + (index * Math.random() * 800),
            y: 0 + Math.random() * 800,
            image
        });
    }),
...
function animate() {
    ...
    if (player.position.y > canvas.height) {
        if (loseCounter === 0) {
            ...
            window.location.href = "./lose3.php";
        }
    }
}
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    function play() {
        var music = new Audio('img/level3.mp3');
        music.play();
    }
...
function updateScrollOffsetDisplay() {
    ...
    scrollOffsetDisplay.textContent = scrollOffset + ' Points (10000 to win)';
}